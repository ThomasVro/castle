...
function scrap(url, result) {
  return new Promise(function(resolve, reject){
    ...
    request({
      ...
      console.log("résultats trouvés: " + countRestaurants);
...
exports.get = function() {
  pagesToScrap.reduce(function(prev, elt, idx, array){
    return prev.then(function(results){
      return scrap(elt, results)
    })
  }, Promise.resolve([]))
  .then(function(results){
    jsonfile.writeFile('liste1.json', results, {spaces: 2}, function(err){
      if(err){
        console.error(err);
      }
      console.log("Nombre de restaurants trouvés: " + results.length);
    });
  });
}
...
exports.getAllAddresses = function(restaurants) {
  ...
  .then(function(restaurants){
    jsonfile.writeFile('liste2.json', restaurants, {spaces: 2}, function(err){
      if(err){
        console.error(err);
      }
    });
